...
const Register = () => {
    ...
    async function save(event) {
        ...
        try {
            ...
            const response = await axios.post("https://mtlbackend.azurewebsites.net/api/v1/employeeManager/register", formData, {
                headers: {
            ...
            if (response.status === 200) {
                // Assuming the backend sends JWT token in response.data.token
                const token = response.data.token;
                localStorage.setItem('jwtToken', token);  // Save token to localStorage

                // Alternatively, you could save it to a cookie
